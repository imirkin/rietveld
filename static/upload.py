...
TEXT_MIMETYPES = ['application/javascript', 'application/x-javascript',
                  'application/xml', 'application/xv+xml',
...
group.add_option("--change", action="store", dest="change",
                 metavar="REV", default=None,
                 help="An already committed revision to review")
...
class MercurialVCS(VersionControlSystem):
  ...
  def __init__(self, options, repo_dir):
    ...
    elif self.options.change:
      self.base_rev = RunShell(["hg", "parent", "-q", "-r", self.options.change]).split(':')[1].strip() + ':' + self.options.change
    else:
